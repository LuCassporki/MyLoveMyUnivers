...
// La fonction appelée par tes boutons de poèmes
window.typePoeme = function (text) {
  if (!text) return;

  stoptxt();

  // Si on change de poème, on reset les index
  //   typedTextSpan.textContent = "";
  //   phraseIndexPoeme = 0;
  //   charIndexPoeme = 0;

  ecritxt(); // On active l'écriture au clic

  startTypeCycle(text);
};
// La fonction appelée par le bouton
function startTypeCycle(text) {
  if (!text || !text[phraseIndexPoeme]) return;
  if (ecriture) {
    if (charIndexPoeme < text[phraseIndexPoeme].length) {
      typedTextSpan.textContent +=
        text[phraseIndexPoeme].charAt(charIndexPoeme);
      charIndexPoeme++;
      setTimeout(() => startTypeCycle(text), typingSpeed);
    } else {
      setTimeout(() => erasePoeme(text), nextPhraseDelay);
    }
  }
}

function erasePoeme(text) {
  if (charIndexPoeme > 0) {
    typedTextSpan.textContent = text[phraseIndexPoeme].substring(
      0,
      charIndexPoeme - 1
    );
    charIndexPoeme--;
    setTimeout(() => erasePoeme(text), erasingSpeed);
  } else {
    phraseIndexPoeme++;
    if (phraseIndexPoeme >= text.length) phraseIndexPoeme = 0;
    setTimeout(() => startTypeCycle(text), typingSpeed + 500);
  }
}
//   #endregion poeme
